...
// Получение досок (обновлённая логика)
if ($action === 'get') {
    // Получаем доски, где пользователь — владелец
    $ownerStmt = $pdo->prepare("SELECT * FROM boards WHERE owner_id = ?");
    $ownerStmt->execute([$user['id']]);
    $ownedBoards = $ownerStmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    $memberBoards = $memberStmt->fetchAll(PDO::FETCH_ASSOC);

    // Объединяем результаты и убираем дубликаты
    $allBoards = array_merge($ownedBoards, $memberBoards);
    $uniqueBoards = [];
    foreach ($allBoards as $board) {
        $uniqueBoards[$board['id']] = $board;
    }

    echo json_encode(array_values($uniqueBoards));
}
